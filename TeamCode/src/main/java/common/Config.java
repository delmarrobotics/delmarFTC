package common;

/**
 * This file contains the name of the hardware used in the Drive Station Configuration file.
 *
 */

public class Config {
    public static final String LEFT_FRONT       = "leftFront";      // port 0
    public static final String RIGHT_FRONT      = "rightFront";     // port 1
    public static final String LEFT_BACK        = "leftRear";       // port 2
    public static final String RIGHT_BACK       = "rightRear";      // port 3

    public static final String COLOR_SENSOR     = "colorSensor";
    public static final String WEBCAM           = "Webcam 1";

    public static final String LIFTING_WENCH    = "lifter";
    public static final String PIXEL_DROPPER    = "dropper";

    public static final String HANGING_SERVO    = "hangingArmServo";
    public static final String HANGING_MOTOR    = "hangingArmMotor";
    public static final String HANDING_TOUCH    = "hangingArmUp";

    public static final String IMU             =  "imu";
    public static final String PIXEL_ARM        = "pixelArm";
    public static final String PIXEL_ELBOW      = "pixelElbow";
    public static final String HAND_UPPER       = "handUpper";
    public static final String HAND_LOWER       = "handLower";

    public static final String CAMERA           = "Webcam 1";
}